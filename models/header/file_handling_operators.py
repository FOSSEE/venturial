...
class new_mesh_file_prompt:
    ...
    def draw(self, layout, context):
        ...
        row = layout.row()
        row.enabled = cs.row_en
        row.operator(VNT_OT_select_mesh_filepath.bl_idname,
                     text="Select mesh file path")
        row.prop(cs, "mesh_dict_path", text="")

        row = layout.row()
        row.alignment = 'CENTER'
        row.label(
            text="Clicking OK will remove all objects from scene", icon="ERROR")
    ...
    def execute(self, optr, context):
        ...
        if os.path.isdir(context.scene.mesh_dict_path):
            ...
            getattr(mesh_dictionary_controller(),
                    "dict_initiate")(optr, context)
    ...
    def execute(self, context):
        ...
        if self.check == True:
            ...
            if os.path.isdir(str(Path(self.properties.filepath))):
                context.scene.mesh_dict_path = str(
