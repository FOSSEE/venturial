...
class get_vertices():
    def draw(self, ptr, context):
        ...
        c2.operator(VNT_OT_select_unselect_allverts.bl_idname, icon="STICKY_UVS_LOC", 
                        text=None).select_all = True
        c2.operator(VNT_OT_select_unselect_allverts.bl_idname, icon="STICKY_UVS_DISABLE",
                        text=None).select_all = False
        
        c2.scale_y = 1.25
        c2.alert = True
        c2.operator(VNT_OT_vertactions.bl_idname, icon='REMOVE', text=None).action = 'REMOVE'
        c2.operator(VNT_OT_clearverts.bl_idname, icon="TRASH", text="")
        c2.alert = False

        row = ptr.row()  
        row.scale_y = 1.2
        row.template_list("CUSTOM_UL_verts", "", cs, "vcustom", cs, "vcustom_index", rows=2)